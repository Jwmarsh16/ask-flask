...
from pydantic import field_validator        # <-- ADDED: validators for trimming/validation
...
class CreateSessionRequest(BaseModel):
    title: Optional[str] = Field(default=None, max_length=200)  # optional title with a sensible max

    @field_validator("title", mode="before")  # <-- CHANGED: run BEFORE built-in constraints
    @classmethod
...
class UpdateSessionRequest(BaseModel):  # <-- ADDED: DTO for PATCH /api/sessions/:id
    title: str = Field(..., min_length=1, max_length=200)  # required; enforce length bounds

    @field_validator("title", mode="before")  # <-- CHANGED: ensure whitespace-only becomes invalid BEFORE min_length
    @classmethod
...
class AppendMessageRequest(BaseModel):
    ...
    tokens: Optional[int] = Field(default=None, ge=0)  # optional non-negative token count
